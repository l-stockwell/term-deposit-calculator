...
const Calculator = () => {
  const form = useForm({
    ...
    validate: {
      depositAmount: (value) =>
        value > 0 ? null : "The deposit amount must be greater than zero.",
      interestRate: (value) =>
        value >= 0 && value <= 15
          ? null
          : "The interest rate must be between 0 and 15%",
      investmentTermInYears: (value) =>
        value >= 0 && value <= 5
          ? null
          : "The investment term in years must be between 0 and 5",
      investmentTermInMonths: (value) =>
        value >= 0 && value <= 12
          ? null
          : "The investment term in months must be between 0 and 12",
      paymentFrequency: (value) =>
        value ? null : "Please select a payment frequency.",
    },
  });
  ...
  return (
    <>
      ...
      {result !== null && (
        <Stack mt="lg">
          <Text size="xl">
            Based on our calculations, your balance will be{" "}
