...
/**
 * Validates inputs for compound interest calculation.
 *
 * @param {CalculatorValues} values - Input values include interest rate, investment term, frequency, and deposit amount.
 * @throws {Error}
 */
function validateInput(values: CalculatorValues): void {
  const {
    interestRate,
    investmentTermInYears,
    investmentTermInMonths,
  ...
  if (investmentTermInYears < 0 || investmentTermInMonths < 0)
    throw new Error("Investment term cannot be negative.");
}
